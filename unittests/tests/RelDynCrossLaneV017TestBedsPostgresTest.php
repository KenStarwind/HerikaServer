...
final class RelDynCrossLaneV017TestBedsPostgresTest extends TestCase
{
     ...
     * the player's next word to each is care ("take my hand"), scored by the eval. That is two
     * things at once, each hers: the rescue response lands on the floor (MDD 3.3, by attachment:
     * Aela, who does not need saving, least), and the moment lands on top of it as a spike
     * (the rescue trigger, by temperament and arousal). The floor keeps exactly what the rescue
     * gave it; the moment is not floor. Alone with him right after, her romantic urge (MDD 13.1,
     * passion x privacy) reads the two together, the effective passion, not the floor alone.
     */
    public function testTheRescueIsAFloorAndAMomentAndTheUrgeReadsBoth(): void
    {
        ...
        $why = json_encode(compact('bonus', 'gain', 'spike', 'floor', 'pre'));
        $this->probe('rescue', compact('bonus', 'gain', 'spike', 'floor', 'pre'));
        foreach ($beds as $npc) {
            $this->assertGreaterThan(0.0, $gain[$npc], "{$npc}: the rescue lands on the floor {$why}");
            $this->assertGreaterThan(0.0, $spike[$npc], "{$npc}: and the moment on top of it {$why}");
            // The floor is the rescue's (and nothing of the moment): the eval asked no passion of its own
            $this->assertEqualsWithDelta($pre[$npc] + $gain[$npc], $floor[$npc], 0.05, "{$npc}: the moment is not floor {$why}");
        }
