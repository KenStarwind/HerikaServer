...
class RelationshipDynamics
{
    ...
    /**
     * The interest the NPC's current place speaks to: the strongest of the 11 interest facets
     * of RelDynFacets::placeFacets() for the core place (eventlog location context + core
     * locations row), or null when core has no location or no interest facet is strong
     * enough (place_facets.dominant_interest_min). A single label for the love-language
     * passion multiplier; the full vector is RelDynFacets::placeFacets().
     * (MinAI's locationkeywords / issneaking / isonmount / sitstate / isinterior keys are not
     * part of CHIM 3.4.1: sneaking, riding and sitting are unknown and not guessed.)
     */
    public static function detectCurrentInterest()
    {
        $npcName = $GLOBALS['RELDYN_NPC_NAME'] ?? $GLOBALS['HERIKA_NAME'] ?? '';
        if (empty($npcName)) return null;
        return RelDynFacets::dominantInterest(RelDynFacets::placeFacets(RelDynFacets::currentPlaceContext($npcName)));
    }
    ...
    public static function calculateInterestSatisfaction(string $npcName, array &$dynamics, ?string $currentInterest, array $eventContext = []): array
    {
        ...
        // Source 3: the current core place (read from the eventlog / locations table, so it
        // works in prerequest, before core sets CACHE_LOCATION). Every interest facet of the
        // place at or above place_facets.satisfies_interest_min satisfies that interest (0.5).
        $placeFacets = RelDynFacets::placeFacets(RelDynFacets::currentPlaceContext($npcName));
        $satisfiesMin = floatval(RelDynFacets::placeFacetConfig()['satisfies_interest_min'] ?? 0.5);
        foreach (RelDynFacets::INTERESTS as $interest) {
            if (floatval($placeFacets[$interest] ?? 0.0) >= $satisfiesMin && isset($interests[$interest])) {
