 ...
 *        - no walkaway, open conflict, ick or resentment withdrawal (block_states);
 *        - the preference filter (getBlockedTypes: demisexual / asexual / aromantic);
 ...
 *          friendzoned NPC), tier ceiling at least the step's, and for 'romantic' the
 *          sociological pillars too (MDD 2.6: visceral only = crush, commitment blocked).
...
final class RelDynRomance
{
    ...
    /** Attraction tier ceilings (calculateTierCeiling), lowest first. */
    const CEILING_ORDER = ['stranger', 'crush', 'friend', 'close', 'bonded', 'sworn'];
     ...
     * core affinity, ceilings are CEILING_ORDER names, multipliers unitless.
     */
        ...
        return [
            ...
            'ladder' => [
                ['to' => 'crush',
                 ...
                 'min_tier' => 'close_friend', 'min_ceiling' => 'crush', 'requires_sociological' => false],
                ['to' => 'romantic',
                 'from' => ['crush', 'admirer'],
                 'min_tier' => 'bonded', 'min_ceiling' => 'close', 'requires_sociological' => true],
            ],
            ...
            'block_states' => ['walkaway', 'conflict', 'ick', 'withdrawn'],
        ];
    }
    ...
    public static function gate(string $npcName, array $dynamics, ?array $cfg = null): array
    {
        ...
        if (in_array($to, RelationshipDynamics::getBlockedTypes($dynamics, $aff), true)) {
            return $closed("preference '" . ($dynamics['relationship_preference'] ?? '') . "' blocks {$to}", $from, $to);
        }
    ...
    public static function buildState(string $npcName, array $dynamics, ?string $coreType, ?int $sharmatArousal): array
    {
        ...
        $matrix = is_array($dynamics['_attraction_matrix_cache'] ?? null) ? $dynamics['_attraction_matrix_cache'] : null;
        ...
        return [
            ...
            'attraction_pass' => $matrix === null ? null
                : (!empty($matrix['visceral_pass']) && !empty($matrix['gender_pass'])),
